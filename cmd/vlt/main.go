...
func newDispatcherAt(configDirectory string, stdin io.Reader, stdout, stderr io.Writer) *cli.Dispatcher {
	...
	completionVault := vaultexec.NewOSCompletionExecutor()
	...
	return cli.NewDispatcher(cli.Dependencies{
		...
		Completion: cli.NewCompletionHandler(cli.CompletionDependencies{Profiles: profiles, Favorites: favorites, Credentials: credentials, Vault: completionVault, PathTransport: http.DefaultTransport, Output: stdout}),
		Vault:      delegate,
	})
}
