...
| 28. Color only profile-linked rows and label picker columns while keeping plain and semantic output | `TestProfileListPresentationPlainAndStyledAreExact`, `TestProfileShowPresentationPlainAndStyledAreExact`, `TestSharedSelectorColorsEachProfileRowAndKeepsSelectionVisible`, `TestFavoriteListColorsRowsByTheirOwnProfiles`, `TestSharedFavoriteSelectorColorsEachRowFromItsOwnProfile`, `TestSharedSelectorShowsProfileAndFavoriteColumnHeaders`, `TestTerminalDisablesColorForAnyNonEmptyNoColorValue`, and the follow-up terminal check above |
