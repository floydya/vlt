...
An isolated v0.2.0 fixture was read successfully by both the released and new builds. A favorite update with the new build wrote version 2 metadata; the v0.2.0 build then rejected it with `decode favorites: unknown field`. Both builds also read the current local profile and favorite metadata without errors. No current local metadata was changed. Rollback to v0.2.0 after a favorite write requires restoring a private pre-upgrade copy of `favorites.json` and loses later favorite changes.
...
| 28. Show per-profile colors only on identity while keeping plain and semantic output | `TestProfileListPresentationPlainAndStyledAreExact`, `TestProfileShowPresentationPlainAndStyledAreExact`, `TestSavedProfileColorDoesNotTintSharedUI`, `TestSavedProfileColorOnlyMarksIdentityInListAndDetails`, `TestSavedProfileColorOnlyMarksIdentityInPickers`, `TestInteractiveFormsAndConfirmationsUseSemanticSelection`, and `TestTerminalDisablesColorForAnyNonEmptyNoColorValue` |
