...
2. Review the Release Please pull request and its generated `CHANGELOG.md`.
   For the stable favorite ID release, state that the first favorite write upgrades `favorites.json` to version 2. A v0.2.0 binary can read the file before that write but cannot read it afterward. Include the private backup and rollback limit from the README.
